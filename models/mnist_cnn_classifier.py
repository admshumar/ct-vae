...
class MNISTCNNClassifier(VAE):
    ...
    def define_classifier(self):
        ...
        x = Dense(self.intermediate_dimension, activation='relu')(x)
        if self.enable_batch_normalization:
            x = BatchNormalization()(x)
        if self.enable_dropout:
            x = Dropout(rate=self.dropout_rate, seed=17)(x)

        x = Dense(self.intermediate_dimension // 2, activation='relu')(x)
        if self.enable_batch_normalization:
            x = BatchNormalization()(x)
        if self.enable_dropout:
            x = Dropout(rate=self.dropout_rate, seed=17)(x)

        x = Dense(self.latent_dimension, activation='relu')(x)

        logits_layer = Dense(self.number_of_clusters)(x)

        # probability_layer = Softmax()(logits_layer)

        classifier = Model(self.encoder_mnist_input, logits_layer, name='mnist_cnn_classifier')
        classifier.summary()
